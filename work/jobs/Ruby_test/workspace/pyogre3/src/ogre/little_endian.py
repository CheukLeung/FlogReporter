# ----------------------------------------------------------------
# little_endian.py - Little-endian byte buffer reader and writer.
# ----------------------------------------------------------------
...
class ByteBufferReader(object):
    ...
    def byteswap64(self,val):
        ...
        ((val >> 24) & 0xff0000) | \
        ((val >> 40) & 0xff00) | \
        (val >> 56)) & 0xffffffffffffffff)

    def readS64(self):
        self.align(8)
        (x, y) = unpack(">2I", self.s.read(8))        
        num = (x<<32) | y
        return self.byteswap64(num)
