...
class Back
  ...
  def generate_signal_classes(atable, 
    ...
    begin
      ...
      f.puts "class LINX_SIGNAL(Union):\n"
      f.puts "#{ind(1)}'''\n"
      f.puts "#{ind(1)}LinxAdapter Signal\n"
      f.puts "#{ind(1)}'''\n"
      f.puts "#{ind(1)}_fields_ = [\n"
      f.puts "#{ind(2)}(\"sig_no\", c_uint),"

      @@grove.each do |a,b|
        f.puts "#{ind(2)}(\"#{b["tree"].s_u_name}\", #{a}),\n"
      end
      f.puts "#{ind(1)}]\n"
