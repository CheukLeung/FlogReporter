...
class SignalClass
  ...
  def generate_fields_list(ahash)
    @s << "\n#{ind(1)}_fields_ = [\n"
    #if @signo != 0
    #    @s << "#{ind(2)}'sig_no',\n" 
    #end
    @root.children[@first_child..-1].each do |node| 
      ...
      if not ahash["ARRAY_ARR"].has_key?(path)
        case node.kind
        when 'numeric'
            @s << "#{ind(2)}('#{node.name}', #{node.basetype}),\n" 
        when 'struct'
            @s << "#{ind(2)}('#{node.name}', #{node.s_u_name}),\n" 
        when 'array'
            @s << "#{ind(2)}('#{node.name}', #{node.s_u_name} * #{node.arrsize}),\n" 
  ...
  def generate_signal_class
    ...
    if @signo != 0
      ...
      @s << "#{ind}class #{@root.s_u_name}(Structure):\n"
  ...
  def generate_init
      if @signo != 0
          if @root.children[0].kind.eql?("numeric") && @root.children[0].name.eql?("sig_no")
              @s << "#{ind(1)}def __init__(self):\n"
              @s << "#{ind(2)}super(Structure, self).__init__()\n"
              @s << "#{ind(2)}self.sig_no = #{@signo}\n"
