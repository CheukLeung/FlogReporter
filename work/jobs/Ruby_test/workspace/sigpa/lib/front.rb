...
# Extended by jf2508
# For more information, see readme front.rb.txt
...
class Front
  ...
  def self.process_header_files
    ...
    loop do
      ...
      # Remove c99 keywords from parser input, extended by jf2508
      next if cpp_str.sub!(/^__extension__/, '')	      
      #next if cpp_str.sub!(/^__attribute__.*/, '')	      
      next if cpp_str.sub!(/__attribute__.*/, ';')	      
      next if cpp_str.sub!(/__restrict/, '')             
      next if cpp_str.sub!(/__asm__.*/, '')	            
      break
    end
  ...
  def self.cparse(preprocessed_header_files)
    ...
    begin
      ...
      #if @@options.debug
         # Errorfile containing found (if any) parse errors, extended by jf2508
