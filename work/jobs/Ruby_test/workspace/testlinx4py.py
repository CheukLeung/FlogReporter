...
class Test(unittest.TestCase):
	...
	def setUp(self):
		self.linx = linx.Linx(TESTSERVER)
		self.linx.addUnionType(LINX_SIGNAL)

		# Hunt for ABSFL model
		self.pid_ABSFL = self.linx.hunt(ABSFL, TIMEOUT)

	def tearDown(self):
		pass

	def test0_ABSReq(self):
		# Sending input signal to ABSFL
		sig_send_ABSFL = ABSFL_INPUT_SIG()
		...
		sig_recv_ABSFL = self.linx.receive(TIMEOUT, [1, ABSFL_OUTPUT_SIG_NO])
		...
		self.assertTrue(sig_recv_ABSFL.states[0].v > 0)
		print "check if v < 40 at tranistion 2"
		self.assertTrue(sig_recv_ABSFL.states[1].v < 40)
		print "check if torqueABS == 0 at tranistion 2"
		self.assertTrue(sig_recv_ABSFL.states[1].torqueABS == 0)
		print 
